  ...
  try {
    ...
    for (var order in orders) {
      ...
      if (productSnapshot.exists) {
        final int currentStock = productSnapshot['stock'] as int;
        if (currentStock > 0) {
    ...
    // Add history entry
    final totalPrice = orders.fold(0, (sum, order) => sum + (order['price'] as int));
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Gagal melakukan pembelian: ${e.toString()}')),
    );
  }
},
                      ...
                      child: const Text('Beli', style: TextStyle(fontSize: 18)),
                    ),
                    ...
                    ElevatedButton(
                      onPressed: () async {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text('Keranjang berhasil dibersihkan')),
                        );
                      },
                      style: ElevatedButton.styleFrom(
                      ...
                      child: const Text('Bersihkan Keranjang', style: TextStyle(fontSize: 18)),
                    ),
                  ],
                ),
              ),
